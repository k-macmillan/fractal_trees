...
Our code for computing the cylinder vertices given the Lindenmayer graphics string (which we've chosen to call L-strings) is shown below.\footnote{We would normally not provide this much detail, but the discussion about optimizing the Blender scene creation later depends on understanding the nontrivial data structures it operates on.}
