...
class DrawingTurtle(Turtle):
    ...
    def __init__(self, _linewidth, _materialindex):
        ...
        if (
            scene.bool_recreate_default_meshes
            and default_internode_mesh_name in bpy.data.meshes.keys()
        ):
    ...
    def push(self):
        ...
        # push state to stack
        self.stack.append(
            (self.mat.copy(), self.linewidth, self.materialindex, self.current_parent)
        )
    ...
    def draw_module(
        self, mesh, name="Module", scale=Vector((1, 1, 1)), assign_material_by_index=False
    ):
    ...
    def create_default_internode_mesh(self, vertex_count):
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            vertices=vertex_count, radius=cylinder_radius, depth=cylinder_length
        )
