...
import bpy
from fractals.turtle import Turtle, DrawingTurtle
...
def interpret(
    ...
    if dryrun_nodraw:
        t = Turtle(
            default_width, default_materialindex
        )  # turtle base class that doesnt draw
        # print("TURTLE INTERPRETATION DRYRUN")
    else:
        t = DrawingTurtle(default_width, default_materialindex)
    ...
    for cmd in commands:
        ...
        # draw custom object
            ...
            if len(args) == 4:
                t.draw_module_from_custom_object(
                    objname=args[0], objscale=Vector((args[1], args[2], args[3]))
                )
            elif len(args) == 2:
                t.draw_module_from_custom_object(
                    objname=args[0], objscale=Vector((args[1], args[1], args[1]))
                )
            elif len(args) == 1:
        ...
        # query turtle state (heading, left, up or position vector)
            ...
            if len(args) == 4:
                ...
                bpy.context.scene.lstring_for_production = replace_nth(
                    bpy.context.scene.lstring_for_production,
                    r"\?\([^()]*\)",
                    '?("{}",{},{},{})'.format(
                        args[0], t.mat.col[querycol].x, t.mat.col[querycol].y, t.mat.col[querycol].z
                    ),
                    turtle_query_command_count - 1,
                )
